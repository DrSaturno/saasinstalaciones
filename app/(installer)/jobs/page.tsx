    ...
    <div className="mx-auto w-full max-w-[1480px]">
      <div className="rounded-2xl bg-brand-purple px-5 py-6 text-white sm:px-7">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          )) : <span className="text-sm text-white/70">{t("configureZones")}</span>}
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <h2 className="font-medium">{t("availableNow")}</h2>
      ...
      {history.length ? (
        <section className="mt-8">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{history.map((job) => <JobCard key={job.id} job={job} />)}</div>
        </section>
      ) : null}
    </div>
  );
