...
export type AvailabilityCompany = {
  ...
  /** El instalador ve todas las suyas, con el estado de revisión de la empresa. */
  exceptions: {
    id: string;
    startsAt: string;
    endsAt: string;
...
export async function fetchInstallerAvailability(supabase: SupabaseClient<Database>, installerId: string): Promise<AvailabilityCompany[]> {
  ...
  const [{ data: companies }, { data: weekly }, { data: exceptions }] = await Promise.all([
    ...
    supabase.from("installer_unavailability").select("id, company_id, starts_at, ends_at, reason, status, review_note").eq("installer_id", installerId).in("company_id", companyIds).gte("ends_at", new Date().toISOString()).order("starts_at"),
  ]);
  return (companies ?? []).map((company) => ({
    ...
    exceptions: (exceptions ?? []).filter((item) => item.company_id === company.id).map((item) => ({ id: item.id, startsAt: item.starts_at, endsAt: item.ends_at, reason: item.reason, status: item.status, reviewNote: item.review_note })),
  }));
}
