  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon-sm" className="relative" aria-label={`${unreadCount} notificaciones sin leer`}>
      ...
      <DropdownMenuContent align="end" className="w-[min(360px,calc(100vw-24px))] p-0">
        <div className="flex items-center justify-between px-3 py-2.5">
          <DropdownMenuLabel className="p-0 text-sm text-foreground">Notificaciones</DropdownMenuLabel>
          {unreadCount ? (
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="mt-1 block font-mono text-[10px] text-muted-foreground">
                  {new Intl.DateTimeFormat("es-AR", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" }).format(new Date(item.createdAt))}
                </span>
              </span>
            </DropdownMenuItem>
          )) : <p className="px-3 py-10 text-center text-sm text-muted-foreground">No hay novedades todavía.</p>}
        </div>
        <DropdownMenuSeparator className="m-0" />
...
function PushPermissionControl({ vapidPublicKey }: { vapidPublicKey: string | null }) {
  ...
  const toggle = () => startTransition(async () => {
    try {
      if (!vapidPublicKey || !("serviceWorker" in navigator) || !("PushManager" in window)) {
        toast.error("Las alertas push no están disponibles en este dispositivo.");
        return;
      }
      ...
      if (permission !== "granted") {
        toast.error("El navegador no autorizó las notificaciones.");
        return;
      }
      ...
      toast.error(error instanceof Error ? error.message : "No se pudieron activar las alertas.");
    }
  });
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-3 py-2.5">
      <div>
        <p className="text-xs font-medium">Alertas del navegador</p>
        <p className="text-[11px] text-muted-foreground">{vapidPublicKey ? (active ? "Activadas" : "Recibí avisos aunque cierres la app") : "Pendiente de configuración"}</p>
      </div>
