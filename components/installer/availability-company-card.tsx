...
export function AvailabilityCompanyCard({ company, disabled }: { company: AvailabilityCompany; disabled: boolean }) {
  ...
  const addException = () => {
    ...
    startTransition(async () => {
      ...
      // Nace pendiente: recién bloquea la agenda cuando la empresa la aprueba.
      setExceptions((current) => [...current, { id: result.id!, startsAt: new Date(startsAt).toISOString(), endsAt: new Date(endsAt).toISOString(), reason, status: "pending" as const, reviewNote: "" }]);
