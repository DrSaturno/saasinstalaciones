  ...
  return (
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent>
          {data.months.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">{t("emptyTrend")}</p> : <div className="flex min-h-48 items-end gap-2 overflow-x-auto pt-8">{data.months.slice(-18).map((month) => <div key={`${month.currency}-${month.month}`} className="flex min-w-14 flex-1 flex-col items-center justify-end gap-2"><span className="font-mono text-caption text-muted-foreground">{format.number(month.value, { notation: "compact", maximumFractionDigits: 1 })}</span><div className="w-full max-w-14 rounded-t-md bg-primary/80" style={{ height: `${Math.max(6, (month.value / maxMonth) * 120)}px` }} /><span className="font-mono text-caption text-muted-foreground">{month.month.slice(2)} · {month.currency}</span></div>)}</div>}
        </CardContent>
      </Card>
    </div>
  );
